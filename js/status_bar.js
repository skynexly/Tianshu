...
async function _renderCustomAttrs(status) {
  ...
  const canShowCustomAttrs = skin === 'neumorph' || skin === 'terminal' || document.body?.getAttribute('data-skin') === 'single-default' || (typeof _isHeartSim === 'function' && _isHeartSim());
  ...
  const attrs = (typeof _isHeartSim === 'function' && _isHeartSim())
    ? [{ id: 'hs_money', name: '金钱', initial: 5000, max: '' }]
    : (gp?.globalAttrs || []).filter(a => a && a.id && (a.name || '').trim());
...
// v719：渲染自定义状态栏组件（直接铺在自定义属性区下方，无区标题；组件标题用用户配置）
async function _renderStatusComponents(status) {
  const wrap = _el('sb-status-components');
  if (!wrap) return;
  const wv = await _getCurrentWorldview();
  const skin = _resolveSkin(wv?.statusBarSkin || document.body?.getAttribute('data-sb-skin') || '');
  const canShow = skin === 'neumorph' || skin === 'terminal' || document.body?.getAttribute('data-skin') === 'single-default' || (typeof _isHeartSim === 'function' && _isHeartSim());
  // 定义表（对话级副本优先）
  let comps = [];
  try {
    const conv = (typeof Conversations !== 'undefined') ? Conversations.getList().find(c => c.id === Conversations.getCurrent()) : null;
    comps = (conv && Array.isArray(conv.convStatusComponents)) ? conv.convStatusComponents : (Array.isArray(wv?.statusComponents) ? wv.statusComponents : []);
  } catch(_) { comps = Array.isArray(wv?.statusComponents) ? wv.statusComponents : []; }
  comps = comps.filter(c => c && c.type && (c.title || '').trim());
  if (!canShow || !status || !comps.length) {
    wrap.classList.add('hidden');
    wrap.innerHTML = '';
    return;
  }
  const cc = status.customComponents || { fields: {}, charRoles: {}, userRoles: {} };
  const fields = cc.fields || {};
  const charRoles = cc.charRoles || {};
  const userRoles = cc.userRoles || {};
  const useCarousel = _useNpcCarousel();
  const PLACEHOLDER = '—';

  // v719：按类型固定视觉顺序——文本 → 用户状态 → 数值 → 相关角色（同类型保持用户配置顺序）
  const _typeOrder = { text: 0, userRole: 1, number: 2, charRole: 3 };
  const sortedComps = comps
    .map((c, i) => ({ c, i }))
    .sort((a, b) => {
      const oa = _typeOrder[a.c.type] ?? 99;
      const ob = _typeOrder[b.c.type] ?? 99;
      return oa !== ob ? oa - ob : a.i - b.i;
    })
    .map(x => x.c);

  // 同类型内的奇偶序号：给每个 block 打 sb-comp-odd / sb-comp-even，供美化做单双数差异样式
  const _typeCount = {};
  const blocks = sortedComps.map((c, ci) => {
    const title = (c.title || '').trim();
    _typeCount[c.type] = (_typeCount[c.type] || 0) + 1;
    const oe = (_typeCount[c.type] % 2 === 1) ? 'sb-comp-odd' : 'sb-comp-even';
    switch (c.type) {
      case 'text': {
        // 文本组件：标题 + 卡片包整段文字（独立样式，不复用皮肤专属 class）
        const val = (fields[title] || '').trim() || PLACEHOLDER;
        return `<div class="sb-comp-block sb-comp-text ${oe}">
          <div class="sb-comp-title">${_esc(title)}</div>
          <div class="sb-comp-textcard${val === PLACEHOLDER ? ' sb-comp-empty' : ''}">${_esc(val)}</div>
        </div>`;
      }
      case 'number': {
        // 数值组件：标题 + 值卡片
        const val = (fields[title] || '').trim() || PLACEHOLDER;
        return `<div class="sb-comp-block sb-comp-number ${oe}">
          <div class="sb-comp-numcard">
            <div class="sb-comp-numcard-name">${_esc(title)}</div>
            <div class="sb-comp-numcard-val${val === PLACEHOLDER ? ' sb-comp-empty' : ''}">${_esc(val)}</div>
          </div>
        </div>`;
      }
      case 'charRole': {
        // 相关角色组件：一个角色一张卡；neumorph/心动=横滑，terminal=折叠竖排
        const roleNames = Object.keys(charRoles).filter(rn => charRoles[rn] && charRoles[rn][title] !== undefined);
        const cards = roleNames.map(rn => {
          const val = (String(charRoles[rn][title] ?? '')).trim() || PLACEHOLDER;
          return `<div class="sb-comp-rolecard">
            <div class="sb-comp-rolecard-name">${_esc(rn)}</div>
            <div class="sb-comp-rolecard-val${val === PLACEHOLDER ? ' sb-comp-empty' : ''}">${_esc(val)}</div>
          </div>`;
        }).join('');
        const inner = cards || `<div class="sb-comp-rolecard sb-comp-rolecard-empty">${PLACEHOLDER}</div>`;
        return `<div class="sb-comp-block sb-comp-charrole ${oe}">
          <div class="sb-comp-title">${_esc(title)}</div>
          <div class="sb-comp-rolelist${useCarousel ? ' carousel' : ''}">${inner}</div>
        </div>`;
      }
      case 'userRole': {
        // 用户状态组件：field 行式（标题 + 值，与玩家衣着/姿势同调），一个面具一行
        const maskNames = Object.keys(userRoles).filter(mn => userRoles[mn] && userRoles[mn][title] !== undefined);
        const rows = (maskNames.length ? maskNames : ['']).map(mn => {
          const val = mn ? ((String(userRoles[mn][title] ?? '')).trim() || PLACEHOLDER) : PLACEHOLDER;
          const label = mn ? `${title}·${mn}` : title;
          return `<div class="sb-comp-userrow">
            <div class="sb-comp-userrow-label">${_esc(label)}</div>
            <div class="sb-comp-userrow-val${val === PLACEHOLDER ? ' sb-comp-empty' : ''}">${_esc(val)}</div>
          </div>`;
        }).join('');
        return `<div class="sb-comp-block sb-comp-userrole ${oe}">${rows}</div>`;
      }
      default:
        return '';
    }
...
async function _getCustomAttrPromptData() {
  const wv = await _getCurrentWorldview();
  // v632.2：心动模拟有内置机制不支持自定义属性；但放行一条内置「金钱」货币（等同人民币，AI 可 ± 操作）
  if (wv && wv.id === 'wv_heartsim') {
    return { wv, globalAttrs: [{ id: 'hs_money', name: '金钱', initial: 5000, max: '', desc: '本世界通用货币，等同人民币，1 金钱 = 1 元，商品与服务价格按现实人民币购买力衡量。剧情中涉及收支（工资、转账、消费、红包等）时可在 custom-attrs 的 global 里对「金钱」输出增量（正为收入、负为支出）' }], charCards: [] };
  }
  const gp = await _getConvGameplay();
...
// v719：读取当前世界观配置的自定义状态栏组件
async function _getStatusComponents() {
  try {
    // 对话级副本优先（保持与 gameplay 一致的读取策略）
    const conv = (typeof Conversations !== 'undefined') ? Conversations.getList().find(c => c.id === Conversations.getCurrent()) : null;
    if (conv && Array.isArray(conv.convStatusComponents)) return conv.convStatusComponents;
    const wv = await _getCurrentWorldview();
    const list = wv?.statusComponents;
    return Array.isArray(list) ? list : [];
  } catch(_) { return []; }
}

// v719：把用户配置的自定义状态栏组件拼成格式说明，注入 status 块规则
async function formatComponentsPrompt() {
  try {
    const comps = (await _getStatusComponents()).filter(c => c && c.type && (c.title || '').trim());
    if (!comps.length) return '';
    const lines = [];
    lines.push('【自定义状态字段】');
    lines.push('本世界观额外要求你在 `status` 代码块中输出以下自定义字段，与 地点/时间/天气 等内置字段一样，每轮都要完整输出，未变化时照抄上一轮的值。字段格式必须严格按下方约定书写，前缀和标题原样保留，冒号后填写内容。');
    lines.push('');
    comps.forEach(c => {
      const title = (c.title || '').trim();
      ...
      switch (c.type) {
        case 'text':
          fmt = `${title}：<文本内容>`;
          note = '（一段文本，用于展示状态/描述/评价类信息）';
          break;
        case 'number':
          fmt = `${title}：<内容>`;
          note = '（一个值或等级，用于展示可量化/分档的信息，如 ABC 段位、境界等）';
          break;
        case 'charRole':
          fmt = `角色-<角色名>-${title}：<内容>`;
          note = '（针对在场角色输出，<角色名> 替换成实际角色名；有多个角色就分别各输出一行）';
          break;
        case 'userRole':
          fmt = `用户角色-<面具名>-${title}：<内容>`;
          note = '（针对用户当前身份/面具输出，<面具名> 替换成实际面具名）';
          break;
        default:
          return;
      }
      lines.push(`- 「${title}」格式：${fmt} ${note}`);
      if (prompt) lines.push(`  要求：${prompt}`);
    });
    return lines.join('\n');
  } catch(e) { return ''; }
}
...
async function applyCustomAttrsDelta(deltaObj) {
  ...
  const wv = await _getCurrentWorldview();
  // v632.2：心动模拟禁用通用自定义属性；但白名单放行内置「金钱」货币，AI 可 ± 操作，其余一律丢弃（保护内置好感度/黑化值机制）
  if (wv && wv.id === 'wv_heartsim') {
    try {
      const g = deltaObj.global && typeof deltaObj.global === 'object' ? deltaObj.global : {};
      // 只认名为「金钱」的增量（AI 按属性名输出）
      const raw = g['金钱'];
      const d = Number(raw);
      if (!Number.isFinite(d) || d === 0) return false;
      const st = _currentStatus || Conversations.getStatusBar() || {};
      st.customAttrs = st.customAttrs || {};
      st.customAttrs.global = st.customAttrs.global || {};
      const cur = Number(st.customAttrs.global['hs_money'] ?? 5000) || 0;
      st.customAttrs.global['hs_money'] = Math.max(0, cur + d);
      _currentStatus = st;
      try { await Conversations.setStatusBar(st); } catch(_) {}
      try { await render(st); } catch(_) {}
      return true;
...
async function render(status) {
    ...
    // v719：自定义状态栏组件
    _renderStatusComponents(status);
...
function hsApplyRelation(relationObj) {
 ...
 _renderHS();
  ...
  // 编辑初始好感度 + 黑化值（合并弹窗，一次改两个数值）
 async function hsEditTarget(idx) {
 const hs = _getHS();
 if (!hs || !hs.targets[idx]) return;
 const t = hs.targets[idx];
 const curFavor = Math.max(0, Math.min(100, Number.isFinite(Number(t.baseFavor)) ? Number(t.baseFavor) : 0));
 const curDark = Math.max(0, Math.min(100, Number.isFinite(Number(t.dark)) ? Number(t.dark) : 0));
 const res = await _hsEditForm(`调整「${t.name}」的数值`, [
   { key: 'favor', label: '初始好感度（0-100）', value: String(curFavor) },
   { key: 'dark', label: '黑化值（0-100）', value: String(curDark) }
 ]);
 if (!res) return;
 const favorNum = parseInt(res.favor);
 const darkNum = parseInt(res.dark);
 if (isNaN(favorNum) || favorNum < 0 || favorNum > 100 || isNaN(darkNum) || darkNum < 0 || darkNum > 100) {
   UI.showToast('请输入 0-100 的数字'); return;
 }
 t.baseFavor = favorNum;
 // 重新 clamp favor（增量不能让总值越界）
 ...
 // 心动目标数值编辑的双字段弹层（避免连续弹 showSimpleInput 的竞态）
 // fields: [{key, label, value}] 返回 {key: value, ...} 或 null（取消）
 function _hsEditForm(title, fields) {
 return new Promise((resolve) => {
   ...
   const rows = fields.map(f => `
     <div style="margin-bottom:12px">
       <div style="font-size:12px;color:var(--text-secondary);margin-bottom:5px">${_esc(f.label)}</div>
       <input class="hs-editform-input" data-key="${f.key}" type="number" min="0" max="100" value="${_esc(f.value || '')}" style="width:100%;box-sizing:border-box;background:var(--bg-tertiary);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:14px;padding:9px 12px;outline:none">
     </div>`).join('');
   overlay.innerHTML = `
     <div class="modal-content" style="max-width:360px;width:calc(100% - 40px)">
       <h3 style="margin:0 0 14px">${_esc(title || '编辑')}</h3>
       ${rows}
   ...
   overlay.addEventListener('click', (e) => {
     ...
     if (b.dataset.act === 'ok') {
       const out = {};
       overlay.querySelectorAll('.hs-editform-input').forEach(el => { out[el.dataset.key] = (el.value || '').trim(); });
       close(out);
     }
   });
   document.body.appendChild(overlay);
   const first = overlay.querySelector('.hs-editform-input');
...
// 积分
 ...
 <button class="hs-target-action edit" onclick="event.stopPropagation();StatusBar.hsEditTarget(${i})" title="调整数值">${editIcon}</button>
...
"叙事描写锁手机" 与 "phone-lock 代码块" 必须同时发生，缺一不可。` });
  ...
  return {
    render, toggle, editField, editEnv, editCustomAttr, editCharacterAttr, applyCustomAttrsDelta, formatCustomAttrsForPrompt, formatCustomAttrsFormatPrompt, formatCustomAttrsStatePrompt, formatComponentsPrompt, editNPC, addNPC, deleteNPC, saveEdit, closeEdit, refreshFromConv, toggleNpcs,
    ...
    // 心动模拟
    hsApplyRelation, hsApplyTasks, hsApplyPhoneLock, hsPrisonAll, hsSkipTask, hsRefreshTasksFromLatestAI, hsOpenSkipModal, hsCloseSkipModal, hsSelectSkipTask, hsConfirmSkipTask, hsAddTarget, hsRemoveTarget, hsEditTarget, hsGetDarknessWarnings, hsFormatForPrompt, hsCheckClearCondition,
    isPhoneLocked, getPhoneLockInfo, hsForceUnlockPhone, isHeartSim: _isHeartSim,
