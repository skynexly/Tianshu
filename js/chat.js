...
if (!gp) return null;
  ...
  function updateSendButton(isSending) {
    ...
    if (isSending) {
      btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" style="width:18px;height:18px"><rect x="6" y="6" width="12" height="12" rx="2.5" /></svg>';
...
bgImage: conv?.convBgImage || '',
  ...
  function openConvSettingsModal() {
    // 无当前对话时不打开（否则进去也保存不了，会让人以为保存按钮坏了）
    if (!Conversations.getCurrent()) {
      UI.showToast('请先创建或选择一个对话');
      return;
    }
