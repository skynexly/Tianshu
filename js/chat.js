...
// v587：按注入位置分流的扩展条目注入
  ...
  // 节日单独保留旧逻辑（外部处理），这里不含。
  function _getCustomAttrValueForEvent(cond) {
    try {
      const status = Conversations.getStatusBar() || {};
      const wv = (typeof Worldview !== 'undefined' && Worldview.getCurrent) ? Worldview.getCurrent() : null;
      ...
      const attr = (wv.gameplay?.globalAttrs || []).find(a => a && a.id === cond.attrId);
      if (!attr) return null;
  ...
  function _eventAttrConditionsMet(ev) {
    const conds = Array.isArray(ev?.attrConditions) ? ev.attrConditions : [];
    if (!conds.length) return false;
    return conds.every(c => _compareEventAttrValue(_getCustomAttrValueForEvent(c), c.operator || '>=', c.value));
  }
      ...
      for (const ev of events) {
        ...
        if (state === 'active') {
          ...
          if (triggerType === 'attr') {
            ...
            hit = _eventAttrConditionsMet(ev);
          } else {
