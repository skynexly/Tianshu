            ...
            if (_card && _card.extEnabled !== false && singleSettings.enableCardExtended !== false) {
              ...
              if (_hiddenWv) {
                ...
                if (_cardKnow.length > 0 || _cardEvents.length > 0) {
                  const _extInj = _buildExtendedInjections(_cardKnow, messages, _cardEvents, _cardConv ? _cardConv.eventStates : {});
...
// v587：按注入位置分流的扩展条目注入
  ...
  //   - 事件条目（triggerMode='event'）：关键词触发→持续注入→结束关键词关闭（v610）
  // 节日单独保留旧逻辑（外部处理），这里不含。
  function _buildExtendedInjections(knowledges, messages, events, convEventStates) {
    ...
    if (dynamicEnabled.length > 0 || hasEvents) {
      const recent = (messages || []).filter(m => m.role !== 'system').slice(-4);
    ...
    if (hasEvents && convEventStates) {
      for (const ev of events) {
        ...
        if (state === 'active') {
          // 进行中：每轮注入
          out.systemTop.push(`【世界观·事件进行中】\n${ev.name || '事件'}：${ev.content || ''}\n\n（当此事件在剧情中自然结束时，请在回复中包含"${ev.completeKey || '__EVENT_DONE__'}"来标记事件完成。这个标记不要展示给玩家看，用HTML注释包裹：<!-- ${ev.completeKey || '__EVENT_DONE__'} -->）`);
        } else {
          // locked：检查关键词是否命中
    ...
    if (isGameMode) {
      try {
        ...
        if (currentWv) {
          ...
          // v610：showContext 路径也传事件数据（只读，不改状态）
          ...
          try {
            ...
            if (_card && _card.extEnabled !== false && singleSettings.enableCardExtended !== false) {
              ...
              if (_hiddenWv) {
                ...
                if (_cardKnow.length > 0 || _cardEvents.length > 0) {
                  const _extInj = _buildExtendedInjections(_cardKnow, messages, _cardEvents, _cardEvtStates);
                  if (_extInj.systemTop.length > 0) {
                    let firstNonSysIdx = apiMessages.findIndex(m => m.role !== 'system');
                    if (firstNonSysIdx === -1) firstNonSysIdx = apiMessages.length;
                    for (const c of _extInj.systemTop.reverse()) {
