...
// v627：人机恋场景下也启用——只要面具+记忆库里有内容就发
{
  ...
  try {
    ...
    _constraintDepth0.push('【禁止】不要代替{{user}}说话、行动或做决定。不要转述、复述、引用或加工{{user}}上一条消息的内容，不要通过旁白如"你的那句…""你的那声…"等类似表达重复{{user}}说过的话，更不要通过角色重复或反问任何{{user}}的语言，如"……在口中过了一遍""……重复了一遍"等。{{user}}说了什么由{{user}}自己表达，你只需要描写{{user}}行为带来的影响，如NPC的反应、环境变化等。禁止通过描写"{{user}}沉默/没有回应"来跳过{{user}}的行动，需要{{user}}做出反应的部分必须等待回复。如果{{user}}的回复中仅动作无语言，视作其并未说话；若仅语言无动作，也无需扩展{{user}}的动作，忽略即可。不要擅自描写{{user}}未在设定中写明的习惯和喜好（如口味、装修、音乐品味、财力等），不要擅自描写{{user}}的神态、表情、情绪或内心活动。');
    }
