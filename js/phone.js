...
\`\`\`
只输出这个 chat 块（如需打电话可在其后附加一个 \`\`\`call 块），不要输出其它任何内容。${ltOnlineBlock}`;
...
6. 如果角色想要主动结束这通电话，在回复的最末尾追加标记（仅标记，不要加其他文字）：
   ...
   三种标记前端都会在台词渲染完毕后自动挂断通话。${callDownInstruction}`;
...
// ===== 对外接口 =====
  ...
  // 通话
_openCall, _callSendMessage, _callRequestReply, _endCall, _callDoSend, _callDoRefresh, _callDoEnd,
