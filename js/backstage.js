...
systemParts.push('[生图能力]\n你拥有生成图片的能力。当用户要求你画图、生成插画、展示场景图等时，在回复中写 [IMG: English description of the image] 标记（描述必须用英文，50-200词，尽量详细描写画面构图、光影、风格）。前端会自动检测该标记并调用生图API生成图片。\n- 用户不要求时不要主动生成图片\n- 一条回复里可以有多个 [IMG:] 标记\n- 描述要具体，避免抽象概念');
    ...
    try {
      if (typeof Memory !== 'undefined' && Memory.retrieveBackstageNotes) {
        ...
        const bsNotesPrompt = Memory.formatBackstageNotesForPrompt(bsNotes);
...
你可以使用 add_backstage_note / query_backstage_notes / update_backstage_note / delete_backstage_note 来记录和查询用户的真实记忆。
- 后台记忆完全独立于主线剧情，主线 AI 看不到这些记忆
- 后台记忆在所有对话窗口之间互通共享
- 只记录用户亲口说的真实偏好/情绪/习惯，不揣测、不修饰
- 不需要每轮都记，只在用户说了值得记的新信息时使用`);
      }
