...
let pendingImages = [];   // [{base64, name, type}]
  ...
  function _getSettings() {
    ...
    toolsCustomFormat: !!conv?.backstageToolsCustomFormat,   // 默认关（后台改额外输出要求）
...
container.innerHTML = messages.map(m => {
  ...
  // ===== 全屏显示后台（全局，存 localStorage，全对话统一）=====
  const _BS_FULLSCREEN_KEY = 'tianshu_backstage_fullscreen';
  function _getFullscreen() {
    try { return localStorage.getItem(_BS_FULLSCREEN_KEY) === '1'; } catch (_) { return false; }
  }
  function _applyFullscreen() {
    const modal = document.getElementById('backstage-modal');
    if (modal) modal.classList.toggle('backstage-fullscreen-mode', _getFullscreen());
    const btn = document.getElementById('backstage-fullscreen-btn');
    if (btn) {
      const on = _getFullscreen();
      btn.title = on ? '还原窗口' : '全屏';
      btn.innerHTML = on
        ? '<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 3v3a2 2 0 0 1-2 2H3"/><path d="M21 8h-3a2 2 0 0 1-2-2V3"/><path d="M3 16h3a2 2 0 0 1 2 2v3"/><path d="M16 21v-3a2 2 0 0 1 2-2h3"/></svg>'
        : '<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 3H5a2 2 0 0 0-2 2v3"/><path d="M21 8V5a2 2 0 0 0-2-2h-3"/><path d="M3 16v3a2 2 0 0 0 2 2h3"/><path d="M16 21h3a2 2 0 0 0 2-2v-3"/></svg>';
    }
  }
  function toggleFullscreen() {
    const on = !_getFullscreen();
    try { localStorage.setItem(_BS_FULLSCREEN_KEY, on ? '1' : '0'); } catch (_) {}
    _applyFullscreen();
  }
...
const container = document.getElementById('backstage-messages');
  ...
  function _updateSendButton() {
    ...
    if (!btn) return;
    if (isStreaming) {
      btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" style="width:16px;height:16px"><rect x="6" y="6" width="12" height="12" rx="2.5" /></svg>';
...
if (c.onlineName) roleIdentityText += `\n网名：${c.onlineName}`;
            ...
            if (dd || c.faceRef) _roleDrawInfo = { name: c.name, desc: dd, faceRef: c.faceRef || null };
          }
...
if (npc.onlineName) roleIdentityText += `\n网名：${npc.onlineName}`;
                ...
                if (dd || npc.faceRef) _roleDrawInfo = { name: npc.name, desc: dd, faceRef: npc.faceRef || null };
              }
            }
          }
        }
...
let imgPrompt = '[生图能力]\n你拥有生成图片的能力。当用户要求你画图、生成插画、展示场景图等时，在回复中写 [IMG: English description of the image] 标记（描述必须用英文，50-200词，尽量详细描写画面构图、光影、风格）。前端会自动检测该标记并调用生图API生成图片。\n- 用户不要求时不要主动生成图片\n- 一条回复里可以有多个 [IMG:] 标记\n- 描述要具体，避免抽象概念\n- 可选：在描述前用「尺寸 | 」指定画面尺寸（不写默认横图 1024x768）。尺寸可用比例词（1:1/16:9/9:16/4:3/3:4/3:2/2:3）、方向词（square/landscape/portrait）或直接写像素（如 1280x720，宽高各 64~2048）。例：竖构图人物立绘写 [IMG: 9:16 | a girl standing...]，方形头像写 [IMG: 1:1 | ...]。根据画面内容自行选择合适的横竖比例。\n- **锁定角色长相**：想让某个角色保持固定长相（使用该角色的参考图）时，在描述最前面加 `face=角色名 | `。画多个角色时用逗号分隔，如 `face=祁诺,杜蘅焰 | `。角色名必须是下方「已提供的角色生图外观」列表中的准确名字，否则锁脸失效。尺寸段和 face 段顺序随意、都可选，如 `[IMG: face=祁诺 | 9:16 | a girl...]` 或 `[IMG: 9:16 | face=祁诺 | a girl...]`。';
  ...
  if (_roleDrawInfo && _roleDrawInfo.desc) {
    drawLines.push(`- 你自己（${_roleDrawInfo.name}）的外观：\n${_roleDrawInfo.desc}${_roleDrawInfo.faceRef ? `\n（你有锁脸参考图，画自己时可用 face=${_roleDrawInfo.name} 锁定长相）` : ''}`);
  } else if (_roleDrawInfo && _roleDrawInfo.faceRef) {
    drawLines.push(`- 你自己（${_roleDrawInfo.name}）：有锁脸参考图，画自己时用 face=${_roleDrawInfo.name} 锁定长相`);
  }
  try {
  ...
  // 挂载角色（对话级常驻）：注入有生图描述的，让 AI 可以 face= 锁它们的脸
  try {
    if (typeof AttachedChars !== 'undefined' && AttachedChars.resolveAll) {
      ...
      for (const a of (_attached || [])) {
        ...
        if (_ad) drawLines.push(`- 常驻角色「${a.name}」的外观：\n${_ad}`);
        else if (a.faceRef) drawLines.push(`- 常驻角色「${a.name}」（有锁脸参考图，可用 face=${a.name} 锁定长相）`);
      }
    }
  } catch(_) {}
  if (drawLines.length) {
      ...
      if (_gresp.ok) {
        ...
        if (_toc) {
          systemParts.push('【skynex 使用说明·功能目录】\n'
            ...
            + '如果用户问起的功能在目录和资料里都找不到，或者你有不确定的地方，不要猜测或编造，请让用户直接去询问作者。\n\n'
            + _toc);
        }
          ...
          if (_userText) {
            ...
            if (_picked.length > 0) {
              ...
              systemParts.push('【功能命中·参考信息】\n'
                ...
                + '如果用户的问题在资料中没有明确说明，问起的功能未在资料中提到，或者你有不确定的地方，不要猜测或编造，请让用户直接去询问作者。\n\n'
                + _blocks);
            }
          }
        } catch(_) {}
...
- 使用 query_directive 可以查看当前是否有生效中的引导`);
      ...
      if (bsSettings.toolsCustomFormat) {
        guideParts.push(`【额外输出要求工具】
你可以使用 set_custom_format / query_custom_format / remove_custom_format 来调整主线 AI 每轮的额外输出要求。
- 额外输出要求会追加在内置回复格式之后，长期生效（不像剧情引导有轮数限制），直到被修改或清空
- 用于让主线 AI 每轮额外输出/遵守某种格式（如心声、旁白、特定排版等）
- **使用前必须向用户确认内容，不要自行决定**
- 使用 query_custom_format 可以查看当前的额外输出要求`);
      }
...
- read_gameplay_config / update_gameplay_config：读写玩法配置（属性/任务/历法 + 手机 App 配置，含电台/直播玩法）。先 read 看结构再按点路径改
- list_event_settings / read_event_setting / add_event_setting / update_event_setting / delete_event_setting：管理剧情事件（关键词触发的独立事件 + 事件链）。先 list 定位 id/chainId 再改
- read_card / update_card：读写当前单人卡
- create_card：新建单人卡并自动挂载为当前对话的常驻角色（建完可用 read_card/update_card 继续改）
...
**用「常驻条目」给主线加提示词：**
用 add_extension_entry（kind:'resident'）新增的常驻条目会【无条件、持续】注入给主线 AI 的世界观知识里，不需要关键词触发。所以想给主线剧情长期加一条设定/规则/背景/氛围指引（比如"最近城里在下连绵的雨""某个禁忌不能提"），就用常驻条目——它会一直挂在主线上下文里生效。content 写清楚这条设定是什么、希望主线怎么体现。（对比：kind:'dynamic' 的动态条目要 keys 关键词命中才注入，适合"只在聊到某事时才出现"的设定。）

**用「事件」影响主线剧情发展：**
用 add_event_setting 新增的剧情事件是【埋在剧情里的触发式导演指令】——当玩家的话或剧情里出现 keys 关键词时，事件被激活，它的 content 会作为导演指令注入主线 AI，指导接下来这段剧情怎么演（引入什么人、发生什么冲突、往哪个方向走），主线演完后输出 completeKey 关闭事件。所以想让主线在某个时机往某个方向发展（埋伏笔、触发转折、安排某人登场），就用事件：keys 选玩家自然会说到或剧情自然会出现的词，content 用"对主线 AI 说话"的导演口吻写清接下来该发生什么。（想串成连续剧情就用事件链 chainId。）`);
      }
...
（手机信息查询目前只覆盖这几个 App，其它 App 暂不支持。）`);
    ...
    const historyForAPI = historyMsgs.map(m => ({ role: m.role, content: (() => { try { return typeof m.content === 'string' ? Utils.stripImgArtifacts(m.content) : m.content; } catch(_) { return m.content; } })() }));
    // 用户消息拼时间戳（后台默认开）
...
try { stampedHistory = TimeAwareness.stampUserMessages(historyForAPI, historyMsgs); } catch(e) {}
      ...
      // 自定义提示词注入（作用域含 backstage）：system_top/bottom 拼进 systemParts，depth 稍后按对话深度插入
      let _bsInjections = { systemTop: [], systemBottom: [], depths: {} };
      const _bsTopMsgs = [];    // 带 role 的 system_top 独立消息
      const _bsBottomMsgs = []; // 带 role 的 system_bottom 独立消息
      try {
        if (typeof Prompts !== 'undefined' && Prompts.buildInjections) {
          _bsInjections = await Prompts.buildInjections('backstage');
          if (_bsInjections.systemTop.length > 0) {
            const _t = [];
            for (const it of _bsInjections.systemTop) {
              const _c = (it && typeof it === 'object') ? it.content : it;
              // 顶部强制 system（拼进 system 大块，不支持独立角色）
              _t.push(_c);
            }
            if (_t.length > 0) systemParts.unshift(..._t);
          }
          if (_bsInjections.systemBottom.length > 0) {
            const _b = [];
            for (const it of _bsInjections.systemBottom) {
              const _c = (it && typeof it === 'object') ? it.content : it;
              const _r = (it && typeof it === 'object' && (it.role === 'user' || it.role === 'assistant')) ? it.role : 'system';
              if (_r === 'system') _b.push(_c); else _bsBottomMsgs.push({ role: _r, content: _c });
            }
            if (_b.length > 0) systemParts.push(..._b);
          }
        }
      } catch(e) { console.warn('[Backstage] 提示词注入失败', e); }
    ...
    // 带 role 的 system_bottom 独立消息注入：system 大块之后、聊天历史之前（splice 到 index 1）
    if (_bsBottomMsgs.length > 0) apiMessages.splice(1, 0, ..._bsBottomMsgs);
    ...
    // 自定义提示词注入 depth 分流：按聊天深度插入（depth 0=最新消息前）
    try {
      if (_bsInjections.depths && Object.keys(_bsInjections.depths).length > 0) {
        for (const [depthStr, contents] of Object.entries(_bsInjections.depths)) {
          ...
          if (insertIdx > 0 && insertIdx <= apiMessages.length) {
            for (const c of [...contents].reverse()) {
              ...
              const _role = (c && typeof c === 'object' && (c.role === 'user' || c.role === 'assistant')) ? c.role : 'system';
              apiMessages.splice(insertIdx, 0, { role: _role, content: _content });
            }
          }
        }
      }
    } catch(e) { console.warn('[Backstage] depth 注入失败', e); }
        ...
        const _enabledTools = (() => {
          ...
          if (typeof Tools !== 'undefined') {
            ...
            merged = allDefs.filter(d => {
              ...
              if (name.includes('custom_format')) return bsSettings.toolsCustomFormat;
...
await DB.del('messages', m.id);
  ...
  function openPromptEdit() {
    ...
    const toolsCfEl = document.getElementById('backstage-tools-custom-format');
    if (toolsCfEl) toolsCfEl.checked = settings.toolsCustomFormat;
...
return String(s == null ? '' : s).replace(/[&<>"']/g, c => map[c]);
  ...
  async function savePrompt() {
    ...
    const toolsCfEl = document.getElementById('backstage-tools-custom-format');
    if (toolsCfEl) conv.backstageToolsCustomFormat = toolsCfEl.checked;
    const toolsWvEl = document.getElementById('backstage-tools-worldview');
  ...
  // 按角色名查 faceRef（后台 AI 生图用）—— 身份角色/面具/挂载/NPC
  async function _getFaceRefsByNamesBackstage(names) {
    const result = {};
    ...
    try {
      if (settings.roleId && settings.roleType) {
        if (settings.roleType === 'card') {
          const c = await SingleCard.get(settings.roleId);
          if (c && c.name && nameSet.has(c.name) && c.faceRef) result[c.name] = c.faceRef;
        } else if (settings.roleType === 'wv_npc') {
          const wvId = settings.roleSourceWvId;
          if (wvId) {
            const wv = await DB.get('worldviews', wvId);
            if (wv) {
              ...
              if (!npc) {
                outer: for (const r of (wv.regions || [])) {
                  for (const f of (r.factions || [])) {
                    const _n = (f.npcs || []).find(n => n.id === settings.roleId);
                    if (_n) { npc = _n; break outer; }
                  }
                }
              }
              if (npc && npc.name && nameSet.has(npc.name) && npc.faceRef) result[npc.name] = npc.faceRef;
            }
          }
        }
      }
    ...
    try {
      ...
      if (_mask && _mask.name && nameSet.has(_mask.name) && _mask.faceRef) result[_mask.name] = _mask.faceRef;
    } catch(_) {}
    // 3) 挂载角色
    try {
      if (typeof AttachedChars !== 'undefined' && AttachedChars.resolveAll) {
        const _attached = await AttachedChars.resolveAll();
        for (const a of (_attached || [])) {
          if (a && a.name && nameSet.has(a.name) && a.faceRef && !result[a.name]) result[a.name] = a.faceRef;
        }
      }
    } catch(_) {}
    // 4) NPC（当前世界观）
    try {
      let allNpcs = [];
      if (typeof NPC !== 'undefined' && NPC.getByRegion) allNpcs = NPC.getByRegion('all') || [];
      for (const nn of allNpcs) {
        if (nn && nn.name && nameSet.has(nn.name) && nn.faceRef && !result[nn.name]) result[nn.name] = nn.faceRef;
      }
    } catch(_) {}
    return result;
  }
  ...
  async function _processImgTagsBackstage(aiMsg) {
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      const parsed = Utils.parseImgTag(m[1]);
      const desc = parsed.desc;
      try {
        // 锁脸：AI 用 face= 声明了角色 → 查这些角色的 faceRef，有则走图生图 edits
        let images;
        const faceMap = await _getFaceRefsByNamesBackstage(parsed.faceNames || []);
        const faceRefs = [];
        const faceRefNames = [];
  ...
  return {
    toggle, minimize, send, cancel, restart, toggleFullscreen,
