...
try { await Gaiden.init(); } catch(e) { console.error('[Gaiden.init]', e); }
  ...
  // ===== 更新公告（登录成功后弹出，可拿到昵称）=====
  try {
    const APP_VERSION = 'v687.41b';
const CHANGELOG = `【v687.41b 多人囚禁结局+世界崩坏演出+鬼屋简化】
· 多人囚禁结局 marker（prison-all）：AI 输出后前端全员黑化拉满+锁手机+崩坏演出
· 世界崩坏演出：全员黑化≥100时裂纹+暗红光晕+抖动（纯视觉，不阻断交互）
· 鬼屋传闻事件简化：3阶段→2阶段，结束条件更明确
· 共同返航动画加点击确认
· 心动模拟防暴力条款
· 好友圈编辑按钮图标修正（edit→pen）
