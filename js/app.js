...
try { await Gaiden.init(); } catch(e) { console.error('[Gaiden.init]', e); }
  ...
  // ===== 更新公告（登录成功后弹出，可拿到昵称）=====
  try {
const APP_VERSION = 'v688.16';
const CHANGELOG = `【v688.16 更新内容】
· 修复：重写AI回复不再清空本轮手机数据（拍照/发圈/刷论坛），只回退剧情副作用
· 修复：生图模型、回复建议模型的「从主模型预设填充」按钮点不动
· 好友圈：收藏动态时一并保存配图
· 好友圈：动态配图新增「存入相册」按钮，可存进手机相册和图库
