...
try { await Gaiden.init(); } catch(e) { console.error('[Gaiden.init]', e); }
  ...
  try {
    const APP_VERSION = 'v691';
      const CHANGELOG = `【v691 更新内容】
· 手机新增日历APP：月视图、自定义历法适配、世界观节日自动填充
· 日历事项管理：支持五种类型（生日/待办/经期/节日/事项）、颜色自选、持续天数、每月/每年重复
· 日历事项编辑/删除、当天日程自动注入提示词提醒
· 第二页Banner卡片：显示游戏时间、日期、天气、最近日程倒计时
· 日历"回到今天"按钮
· 从第二页进入APP返回后保持在第二页
· 状态栏布局设置移至基础tab（绑定主题下方）
· 修复：切换对话后日历状态未重置
· 修复：记忆参与者字段类型异常导致报错`;
