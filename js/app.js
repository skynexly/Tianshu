...
try { await Gaiden.init(); } catch(e) { console.error('[Gaiden.init]', e); }
  ...
  try {
    ...
    const CHANGELOG = `●新增「宠物系统」（小屋 APP → 宠物）
○新增宠物商城、宠物衣橱、宠物喂食器
○宠物获取：手动填写、AI 生成、商城购买
○宠物状态：根据主线剧情与小屋环境推演宠物当前状态，宠物可能待在你家任何一个地方
○变化记录：记录上次刷新到本次刷新之间的状态变化与数值变化原因
○AI 生图：根据宠物外貌生成头像
○宠物衣橱：购买的服装存放于此，可随时给宠物更换
○宠物喂食器：填充宠物食品后，宠物可能自行食用
○携带宠物：携带时向主线发送该宠物的详细资料（未携带只发简介）
●新增一键清空手机数据（设置 → 通用 → 重置手机数据，附清空前找回）
●部分 Bug 修复`;
    // 历史公告（最新在前），版本变旧后手动把上一版内容挪进来
    const CHANGELOG_HISTORY = [
      { version: 'v720', notes: `○优化邮箱 APP：调整排序，新增多个往来合并功能
○新增直播与粉丝群互通：创建粉丝群时优先使用直播间成员，聊天消息可互通（仅用户开播时）
○新增游鱼小铺商品分享至群聊售卖功能
○新增语音／视频通话灵感回复
○新增 GitHub 自动备份（需先配置）
○新增隐藏顶部状态栏功能（对话设置内）
○部分 Bug 修复与 UI 调整` },
      { version: 'v719', notes: `○新增正则作用时机：选择「仅显示」后，正则只影响显示效果、不再替换原文
