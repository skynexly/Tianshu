...
try { await Gaiden.init(); } catch(e) { console.error('[Gaiden.init]', e); }
  ...
  // ===== 更新公告（登录成功后弹出，可拿到昵称）=====
  try {
    const APP_VERSION = 'v688.17';
      const CHANGELOG = `【v688.17 更新内容】
· 后台小纸条：标签选择器改为分组气泡（对齐前台，支持开心/感动等情绪类标签）
· 后台小纸条：删除口令后 tab 消失的 bug 修复；后台图标换为 SVG
· 商店下单：日志追加扣款提示；同轮多笔购物自动计算合计总价
· 心动模拟世界观：禁止编辑对话级自定义属性/事件/任务/骰子
