...
try { await Gaiden.init(); } catch(e) { console.error('[Gaiden.init]', e); }
  ...
  // ===== 更新公告（登录成功后弹出，可拿到昵称）=====
  try {
const APP_VERSION = 'v704.9';
 const CHANGELOG = `【v704.9 更新内容】

🔧 修复：小纸条导出为空
○ 记忆导出功能此前未适配小纸条，导出内容为空，现已修复（支持导出/导入）

🔧 修复：事件界面卡死
○ 部分事件因提取掉格式导致事件列表整个打不开，现已修复并对异常数据降级显示

🔧 修复：奶油软糖主题下部分显示
○ 钱包、游鱼、飞鸟、小屋的添加按钮文字在奶油软糖主题下看不清，现已修复`;
