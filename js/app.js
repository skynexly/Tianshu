...
try { await Gaiden.init(); } catch(e) { console.error('[Gaiden.init]', e); }
  ...
  // ===== 更新公告（登录成功后弹出，可拿到昵称）=====
  try {
    const APP_VERSION = 'v696.0';
const CHANGELOG = `【v696.0 更新内容】

新增功能
· 角色自主性
  ○ 对话设置→AI行为约束新增「角色自主性」开关，让角色保有独立的生活、目标与判断，相处与亲近循序渐进
· 气泡渲染 HTML
  ○ AI 回复中的 HTML/SVG 内容现在会在聊天气泡里直接渲染，可用于富文本排版与样式
· 过滤历史 HTML
  ○ 对话设置→输出→回复格式新增「过滤历史HTML」开关，发送请求时自动剔除 AI 历史回复里的 HTML，省 token、降噪
  ○ 可选「过滤时保留文字」：开启则只剥标签留文字，关闭则标签连内容一起删
  ○ 仅作用于发送给 AI 的内容，不影响存档原文与气泡显示

优化
· 优化了心动模拟相关提示词

修复
· 修复了切换主题后世界观/世界书/角色编辑页空白的问题
· 修复了语音/视频通话功能的格式错误
· 修复了相机功能输出多余逗号的问题`;
