...
try { await Gaiden.init(); } catch(e) { console.error('[Gaiden.init]', e); }
  ...
  // ===== 更新公告（登录成功后弹出，可拿到昵称）=====
  try {
    const APP_VERSION = 'v692.0';
      const CHANGELOG = `【v692.0 更新内容】
· 新增：手机线上聊天 AI 可发送图片、位置、订单、转账，转账可点击收取到钱包绑定的货币
· 新增：角色可在聊天中打断你看手机，自动收起并转入线下剧情（可在聊天设置中开关）
· 新增：环境音系统，根据天气自动播放雨/风/雷/底噪（对话设置·多媒体）
· 新增：聊天可自定义发送历史条数（默认20）
· 优化：线上聊天自动推进状态栏时间，时间格式全面统一
· 优化：世界观节日设定按日期自动排序
· 修复：继续剧情会重新生成的问题、主线复述聊天被重复收录的问题`;
