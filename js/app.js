...
try { await Gaiden.init(); } catch(e) { console.error('[Gaiden.init]', e); }
  ...
  // ===== 更新公告（登录成功后弹出，可拿到昵称）=====
  try {
    const APP_VERSION = 'v687.39';
    const CHANGELOG = `【v687.39 叙述协议重写 + 防八股冷却】
· 防转述/防代写换成新版「叙述协议」（带允许示例+正反对照）
· 防八股加冷却机制（触发后3轮内不再注入，避免长期单角色被持续催换写法）
· 面具快捷切换栏显示备注
· （续上版）面具备注栏 / 删分支不删面具 / iOS恢复同步 / 后台cancel锁+think折叠
