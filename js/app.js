...
try { await Gaiden.init(); } catch(e) { console.error('[Gaiden.init]', e); }
  ...
  // ===== 更新公告（登录成功后弹出，可拿到昵称）=====
  try {
    const APP_VERSION = 'v685.3';
const CHANGELOG = `【v685.3 收窄改动】
· 主线工具描述全部回滚回"用户"（主线"用户"=面具角色，不要折腾）
· 只有后台 5 个小纸条工具描述里的"用户"替换成 OOC 昵称
· 因为后台池子可能跨用户共享，避免 AI 被各种面具名整迷糊

【v685.1 后台口令（保留）】
· 后台口令默认 1001，进入后台后右上角可改
