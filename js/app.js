...
try { await Gaiden.init(); } catch(e) { console.error('[Gaiden.init]', e); }
  ...
  // ===== 更新公告（登录成功后弹出，可拿到昵称）=====
  try {
    const APP_VERSION = 'v690.2';
      const CHANGELOG = `【v690.2 更新内容】
· 新增历法系统：可自定义周/月/季/年（世界观→玩法→历法系统）
· 时间改为增量格式：AI只需输出"+30min"等增量，前端自动计算绝对日期
· 状态栏新增季节显示（根据月份自动计算）
· 时间流逝感知提示词重写，默认开启，防止时间推进过快或过慢
· 事件系统新增"时间触发"类型，支持按游戏时间范围触发事件
· 好友圈动态时间修正：严格早于当前游戏时间
· 好友圈支持网名/代号自动转本名再查备注昵称
· 线上聊天(chat块)时间简化为只写时分，前端自动补日期
· 总结/记忆提取/归档消息中增量时间自动替换为绝对时间`;
