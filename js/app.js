...
const CHANGELOG = `【v686 骰点系统】
· 对话设置 → 游戏系统 多了「骰点系统」开关
   - 启用前会强制要求先配数值系统（没有属性投不了骰）
   - 可设骰子上限（默认 1d100）和判定规则（默认 ≤ 成功）
· 启用后聊天输入框旁出现 🎲 按钮
   - 选属性 → 看到当前值 → 投掷
   - 投出来不爽可以再投，确认结果后才记账才发给 AI
· AI 在剧情末尾打【检定·力量】标记
   - 前端识别成可点击按钮，点了直接弹投骰面板（属性预选锁定）
· 结果跟着聊天记录走，居中小气泡显示
· 发送时只带最后一次确认的 roll，作为 OOC 拼到消息末尾
