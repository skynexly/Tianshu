...
${_stepIntro('castle', '第 3 步 · 势力', '为地区生成势力组织' + (regionNames ? '（' + regionNames + '）' : ''))}
      ...
      <div class="wv-gen-grid">
        ...
        <div class="wv-gen-field">
          ...
          <input id="wv-gen-count" type="number" min="1" max="10" inputmode="numeric" oninput="WvGenerator._enforceCountInput('wv-gen-count', 10, '每地区势力数')" onchange="WvGenerator._enforceCountInput('wv-gen-count', 10, '每地区势力数', true)" onblur="WvGenerator._enforceCountInput('wv-gen-count', 10, '每地区势力数', true)" value="${Math.min(10, Math.max(1, _genData.step3?._count || 3))}" class="wv-gen-input">
        </div>
        <div class="wv-gen-help full">建议 2–4 个，最多 10 个。数量过多会增加后续角色生成压力，建议分批补充。</div>
      </div>
      <div id="wv-gen-batch-progress" class="wv-gen-batch-progress" style="display:none"></div>
      <div id="wv-gen-status" class="wv-gen-status"></div>
      <div class="wv-gen-actions">
  ...
  async function _runStep3() {
    ...
    try {
      ...
      await _upsertWV();  // 渐进落库：第3步（势力）完成即入库
      _setLoading(false);
...
${_stepIntro('users', '第 4 步 · 角色', '为每个势力生成 NPC 角色')}
      ...
      <div class="wv-gen-grid">
        ...
        <div class="wv-gen-field">
          ...
          <input id="wv-gen-count" type="number" min="1" max="10" inputmode="numeric" oninput="WvGenerator._enforceCountInput('wv-gen-count', 10, '每势力角色数')" onchange="WvGenerator._enforceCountInput('wv-gen-count', 10, '每势力角色数', true)" onblur="WvGenerator._enforceCountInput('wv-gen-count', 10, '每势力角色数', true)" value="${Math.min(10, Math.max(1, _genData.step4?._count || 3))}" class="wv-gen-input">
        </div>
        <div class="wv-gen-help full">建议 2–4 个，最多 10 个。角色过多可能导致页面卡顿或生成中断，建议分批追加。</div>
      </div>
      <div id="wv-gen-batch-progress" class="wv-gen-batch-progress" style="display:none"></div>
      <div id="wv-gen-status" class="wv-gen-status"></div>
      <div class="wv-gen-actions">
...
: '';
      ...
      await _upsertWV();  // 渐进落库：第4步（角色·批量路径）完成即入库
      _setLoading(false);
      const totalSucc = results.filter(r => r.status === 'done').length;
      const totalFail = results.filter(r => r.status === 'failed').length;
      if (totalFail > 0) {
