...
${_stepIntro('message', '第 5 步 · 开场剧情', '生成开场时间、剧情引导和第一条消息')}
  ...
  async function _commit() {
    ...
    for (const npc of npcs) {
      const npcObj = {
        name: npc.name || '',
        aliases: npc.alias || '',
        summary: [npc.gender, npc.age, npc.identity].filter(Boolean).join(' · ') || '',
        detail: npc.detail || '',
        avatar: ''
      };
  ...
  async function inlineFactionNpcs() {
    ...
    await _openInlineGenModal({
      ...
      fac.npcs = fac.npcs || [];
      arr.forEach(n => fac.npcs.push({ id: 'npc_' + Utils.uuid().slice(0,8), name: n.name || '', aliases: n.alias || '', summary: [n.gender, n.age, n.identity || n.description].filter(Boolean).join(' · ') || '', detail: n.detail || '', avatar: '' }));
  ...
  async function inlineFillNpc() {
    ...
    await _openInlineGenModal({
      ...
      if (summary && !summary.value.trim()) {
        const sumParts = [n.gender, n.age, n.identity || n.description].filter(Boolean);
        if (sumParts.length) summary.value = sumParts.join(' · ');
      }
    ...
    await _openInlineGenModal({
      ...
      for (const n of arr) {
        ...
        if (target && !target.detail?.trim()) {
          ...
          if ((n.identity || n.description || n.gender || n.age) && !target.summary?.trim()) {
            const sumParts = [n.gender, n.age, n.identity || n.description].filter(Boolean);
            if (sumParts.length) target.summary = sumParts.join(' · ');
          }
        }
      }
      await Worldview._saveEditingWV(w);
