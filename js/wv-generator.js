...
${_stepIntro('castle', '第 3 步 · 势力', '勾选要本次生成势力的地区，一次性合并生成')}
  ...
  function _updateStep3Estimate() {
    ...
    const WARN = 8000;
    if (nRegion === 0) {
      el.className = 'wv-gen-estimate';
      el.innerHTML = '<span class="wv-gen-help">未勾选地区</span>';
      return;
    }
    const base = `已勾 ${nRegion} 个地区 × ${count} 势力 × 约 ${words} 字 ≈ 预计 ~${total} 字`;
    if (total > WARN) {
      el.className = 'wv-gen-estimate warn';
      el.innerHTML = `⚠ ${base}<br>内容较大，一次生成可能导致 JSON 结构错误或截断（能解析多少就存多少）。建议减少勾选地区或降低字数。`;
    } else {
      el.className = 'wv-gen-estimate';
      el.innerHTML = base;
    }
...
${_stepIntro('users', '第 4 步 · 角色', '勾选要本次生成角色的势力（可跨地区），一次性合并生成')}
  ...
  function _updateStep4Estimate() {
    ...
    if (nFac === 0) {
      el.className = 'wv-gen-estimate';
      el.innerHTML = '<span class="wv-gen-help">未勾选势力</span>';
      return;
    }
    const base = `已勾 ${nFac} 个势力 × ${count} 角色 × 约 ${words} 字 ≈ 预计 ~${total} 字`;
    if (total > WARN) {
      el.className = 'wv-gen-estimate warn';
      el.innerHTML = `⚠ ${base}<br>内容较大，一次生成可能导致 JSON 结构错误或截断（能解析多少就存多少）。建议减少勾选势力或降低字数。`;
    } else {
      el.className = 'wv-gen-estimate';
      el.innerHTML = base;
    }
...
${_stepIntro('message', '第 5 步 · 开场剧情', '生成开场时间、剧情引导和第一条消息')}
  ...
  function _updateFacBatchEstimate() {
    ...
    if (nRegion === 0) { el.className = 'wv-gen-estimate'; el.innerHTML = '<span class="wv-gen-help">未勾选地区</span>'; return; }
    const total = nRegion * count * words;
    const base = `已勾 ${nRegion} 个地区 × ${count} 势力 × 约 ${words} 字 ≈ 预计 ~${total} 字`;
    if (total > 8000) {
      el.className = 'wv-gen-estimate warn';
  ...
  async function _renderNpcsBatchModal() {
    ...
    body.innerHTML = `
      ...
      <div class="wv-gen-field">
        <label class="wv-gen-label">额外要求（可选）</label>
        <textarea id="wv-nb-prompt" rows="3" placeholder="对角色有什么要求？留空则由 AI 自由发挥" class="wv-gen-textarea">${Utils.escapeHtml(_npcBatchPrompt)}</textarea>
      </div>
      ${_refLorebookBtnHtml('inline')}
      <div class="wv-gen-field">
  ...
  function _updateNpcBatchEstimate() {
    ...
    if (nFac === 0) { el.className = 'wv-gen-estimate'; el.innerHTML = '<span class="wv-gen-help">未勾选势力</span>'; return; }
    const total = nFac * count * words;
    const base = `已勾 ${nFac} 个势力 × ${count} 角色 × 约 ${words} 字 ≈ 预计 ~${total} 字`;
    if (total > 8000) {
      el.className = 'wv-gen-estimate warn';
