...
${_stepIntro('message', '第 5 步 · 开场剧情', '生成开场时间、剧情引导和第一条消息')}
  ...
  async function inlineFillAllRegions() {
    ...
    await _openInlineGenModal({
      icon: 'globe',
      title: 'AI 填充已有地区',
      desc: `为 ${empty.length} 个空白地区（${empty.map(r=>r.name).join('、')}）批量生成设定`,
      placeholder: '对地区有什么要求？留空则由 AI 自由发挥',
      countLabel: '地区数（自动）',
      defaults: { count: empty.length, wordCount: 300 },
      limits: { count: [empty.length, empty.length], wordCount: [100, 1000] },
      loadingMsg: `正在分批填充 ${empty.length} 个地区…`
    }, async ({ prompt, wordCount, signal }) => {
      // 多次调用提醒
      if (empty.length >= 2) {
        const ok = await UI.showConfirm(
          '一键生成提示',
          `将为 ${empty.length} 个地区分别调用 AI，共 ${empty.length} 次请求。\n按次计费的中转站请留意消耗。\n确定开始？`
        );
        if (!ok) throw new Error('用户取消');
      }
      // 按地区分批串行，避免一次性大批量请求截断
      const results = empty.map(r => ({ name: r.name, status: 'pending' }));
      _renderBatchProgress?.(empty.map(r=>r.name), -1, results);
      let filledTotal = 0;
      for (let i = 0; i < empty.length; i++) {
        const target = empty[i];
        results[i].status = 'running';
        _renderBatchProgress?.(empty.map(r=>r.name), i, results);
        try {
          const sysPrompt = PROMPTS.step2.replace('##WORD_COUNT##', wordCount).replace('##EXISTING_REGIONS##',
            `\n## 必须对齐的地区\n${target.name}`);
          const userMsg = `${prompt.trim() ? '用户要求：' + prompt.trim() + '\n\n' : ''}仅生成 1 个地区「${target.name}」的详细设定。\n\n## 世界观设定\n${setting}`;
          const raw = await API.generate(sysPrompt, userMsg, { signal });
          _lastRawOutput = raw;
          const arr = _parseJSON(raw);
          const r = Array.isArray(arr) ? arr[0] : arr;
          ...
          results[i].status = 'failed';
          results[i].info = e.message?.slice(0, 30) || '失败';
          console.error('[WvGen] 填充地区失败', target.name, e);
        }
        _renderBatchProgress?.(empty.map(r=>r.name), i, results);
      }
      await Worldview._saveEditingWV(w);
      Worldview._renderRegions(w.regions);
      Worldview.switchEditTab('detail');
      UI.showToast(`完成：${filledTotal}/${empty.length}`, 2500);
    });
  }
  ...
  async function inlineFillAllGlobalNpcs() {
    ...
    await _openInlineGenModal({
      icon: 'users',
      title: 'AI 填充已有角色',
      desc: `为 ${empty.length} 个空白角色（${empty.map(n=>n.name).join('、')}）批量生成设定`,
      placeholder: '对角色有什么要求？留空则由 AI 自由发挥',
      countLabel: '角色数（自动）',
      defaults: { count: empty.length, wordCount: 500 },
      limits: { count: [empty.length, empty.length], wordCount: [200, 1500] },
      loadingMsg: `正在分批填充 ${empty.length} 个角色…`
    }, async ({ prompt, wordCount, signal }) => {
      // 多次调用提醒
      if (empty.length >= 2) {
        const ok = await UI.showConfirm(
          '一键生成提示',
          `将为 ${empty.length} 个角色分别调用 AI，共 ${empty.length} 次请求。\n按次计费的中转站请留意消耗。\n确定开始？`
        );
        if (!ok) throw new Error('用户取消');
      }
      const results = empty.map(n => ({ name: n.name, status: 'pending' }));
      _renderBatchProgress?.(empty.map(n=>n.name), -1, results);
      let filledTotal = 0;
      for (let i = 0; i < empty.length; i++) {
        const target = empty[i];
        results[i].status = 'running';
        _renderBatchProgress?.(empty.map(n=>n.name), i, results);
        try {
          const sysPrompt = PROMPTS.step4.replace('##WORD_COUNT##', wordCount);
          const label = target.name + (target.summary ? '（' + target.summary + '）' : '');
          const userMsg = `${prompt.trim() ? '用户要求：' + prompt.trim() + '\n\n' : ''}仅生成 1 个角色「${label}」的详细设定。该角色为常驻角色。\n\n## 世界观设定\n${setting}`;
          const raw = await API.generate(sysPrompt, userMsg, { signal });
          _lastRawOutput = raw;
          const arr = _parseJSON(raw);
          const n = Array.isArray(arr) ? arr[0] : arr;
          if (n && n.detail) {
            target.detail = n.detail;
          ...
          results[i].status = 'failed';
          results[i].info = e.message?.slice(0, 30) || '失败';
          console.error('[WvGen] 填充角色失败', target.name, e);
        }
        _renderBatchProgress?.(empty.map(n=>n.name), i, results);
      }
      await Worldview._saveEditingWV(w);
