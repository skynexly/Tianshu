...
##FEEDBACK##`
  ...
  // 统一处理 AI 输出的 NPC 字段：兼容 alias/aliases，拼出 summary 字符串（gender·age·profession·identity）
...
for (const npc of npcs) {
  ...
  async function inlineFactionNpcs() {
    ...
    await _openInlineGenModal({
      ...
      fac.npcs = fac.npcs || [];
      arr.forEach(n => fac.npcs.push({ id: 'npc_' + Utils.uuid().slice(0,8), name: n.name || '', aliases: _npcAliases(n), summary: _npcSummary(n), detail: n.detail || '', avatar: '' }));
    ...
    await _openInlineGenModal({
      ...
      for (const n of arr) {
        ...
        if (target && !target.detail?.trim()) {
          ...
          if ((n.identity || n.description || n.gender || n.age || n.profession) && !target.summary?.trim()) {
            const sumStr = _npcSummary(n);
