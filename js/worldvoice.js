  ...
  async function _getNpcListForForum() {
    try {
      ...
      const _npcEntry = (n) => {
        ...
        let s = `本名：${n.name}，username填：${username}`;
      ...
      return `\n\n## 世界观中的角色（可作为发帖人/评论者，username字段只填下方指定的名字，不要加括号或其他说明）\n${npcs.join('\n')}`;
    } catch(_) { return ''; }
  }
...
8. 其中需包含1-2条来自世界观中有设定的角色（非路人NPC）的帖子，username 字段只填角色的网名（如角色资料中明确有网名字段）或本名，不得加括号、注释或其他任何说明性文字
...
2. 评论/回复区8-12条，风格多样（赞同、反对、吐槽、跑题等）。长度自然，有人一句话有人写一段。可以加入适量的“@某人”或引用前排回复的互动感，体现出网友间的楼中楼交流（例如“@XX 确实”或“回复 @XX：不对吧”），但不要每条都@。
...
4. 评论时间必须依次晚于该帖子的发帖时间和已有的最新评论。请根据“当前游戏时间”智能安排回复节奏：
   ...
   - 严禁跳跃到不合逻辑的遥远未来。
5. 其中需包含1-2条来自世界观中有设定的角色（非路人NPC）的评论，username 字段只填角色的网名（仅当角色资料中明确有网名字段时）或本名，不得加括号、注释或其他说明，评论内容符合该角色的性格
6. 所有 time 都必须使用"YYYY.MM.DD 星期X HH:mm"格式，不要写成别的时间样式
