...
const Memory = (() => {
  ...
  function _buildEventRelationPrompt(recentMessages, charName, charInfo, extractLimits, existingTitles) {
    ...
    return `${charBlock}【命名规则·强制】提取时所有角色、玩家、NPC、地点、组织等专有名字必须严格保留原文（包括英文名、数字、符号），不得翻译、音译或本地化，并严格保持原有大小写。
请从以下对话中按时间顺序提取所有重要事件和关系变化，按JSON格式输出。只输出JSON，不要其他内容。
...
（命名规则：所有名字必须严格保留原文，包括英文名、数字、符号，不得翻译、音译或本地化，并严格保持原有大小写。）
