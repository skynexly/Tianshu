  ...
  function _defaultWorldview(id) {
    return {
      ...
      phoneApps: {                       // 手机 App 自定义（留空用默认）
        ...
        map:     { name: '', desc: '', currencyIds: [] }, // 地图（默认高德）；desc 写交通工具，注入地图搜索；currencyIds 限定买票货币
      },
...
knowledges: []  // v581：customs 已合并到 knowledges，按 keywordTrigger 字段区分常驻/动态
  ...
  function _buildSettingWithExtras(w) {
    ...
    if (curs.length) {
      ...
      // 商城限定货币：轻量提一句（前端负责扣费与拦截，AI 只需知道即可，不必强调）
      try {
        const _pa = w.phoneApps || {};
        const _attrName = (id) => {
          const a = (w.gameplay?.globalAttrs || []).find(x => x && x.id === id);
          return a ? (a.name || id) : null;
        };
        const _mallLabels = [
          ['takeout', (_pa.takeout?.name || '短时效商城'), _pa.takeout?.currencyIds],
          ['shop', (_pa.shop?.name || '长时效商城'), _pa.shop?.currencyIds],
          ['cottage', (_pa.cottage?.name || '家具商城'), _pa.cottage?.currencyIds],
          ['wardrobe', (_pa.wardrobe?.name || '服装商城'), _pa.wardrobe?.currencyIds],
          ['live', '直播间商城', _pa.video?.liveCurrencyIds],
          ['map', (_pa.map?.name || '地图'), _pa.map?.currencyIds],
        ];
        const _lines = [];
        _mallLabels.forEach(([k, label, ids]) => {
          if (Array.isArray(ids) && ids.length) {
            const names = ids.map(_attrName).filter(Boolean);
            if (names.length) _lines.push(`${label}：${names.join('、')}`);
          }
        });
        if (_lines.length) {
          extra.push('【商城限定货币】部分商城只收指定货币（前端已处理扣费与限制，你了解即可）：');
          _lines.forEach(l => extra.push('- ' + l));
        }
      } catch (_) {}
      s = s ? `${s}\n\n${extra.join('\n')}` : extra.join('\n');
    }
...
function _defaultRegion() {
    return { id: 'reg_' + Utils.uuid().slice(0,8), name: '', summary: '', detail: '', bgImage: '', factions: [] };
  }
...
async function editLorebookDescription() {
 ...
 <div style="flex:1;min-width:0">
  ...
  async function openRegionEdit(ri) {
    ...
    _renderRegionBgPreview(reg.bgImage || '');
 ...
 <div style="flex:1;min-width:0">
  ...
  async function saveRegion(silent) {
    ...
    // 背景图从预览的 data-value 读取（上传时已即时存过，这里兜底同步）
    const _bgImg = document.querySelector('#wv-reg-bg-preview img');
    reg.bgImage = _bgImg ? (_bgImg.dataset.value || _bgImg.src || '') : '';
    await _saveEditingWV(w);
  ...
  // ---------- 地区背景图上传 ----------
  // 进入该地区时聊天界面会自动切换到这张背景图（优先级高于对话级/主题级背景）
  function _renderRegionBgPreview(bgImage) {
    const preview = document.getElementById('wv-reg-bg-preview');
    if (!preview) return;
    if (bgImage) {
      preview.innerHTML = `<img src="${bgImage}" style="width:100%;height:100%;object-fit:cover" data-value="${bgImage}">`;
    } else {
      preview.innerHTML = '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:var(--text-secondary);font-size:12px">未设置背景图</div>';
    }
    const clearBtn = document.getElementById('wv-reg-bg-clear');
    if (clearBtn) clearBtn.style.display = bgImage ? 'inline-flex' : 'none';
  }
  async function handleRegionBgUpload() {
    const dataUrl = await Utils.promptImageInput({ maxSize: 1280, quality: 0.85 });
    if (!dataUrl) return;
    _renderRegionBgPreview(dataUrl);
    // 即时存进当前编辑的地区
    const w = await _getEditingWV();
    if (w && _editRegionIdx >= 0 && w.regions[_editRegionIdx]) {
      w.regions[_editRegionIdx].bgImage = dataUrl;
      await _saveEditingWV(w);
    }
  }
  async function clearRegionBg() {
    _renderRegionBgPreview('');
    const w = await _getEditingWV();
    if (w && _editRegionIdx >= 0 && w.regions[_editRegionIdx]) {
      w.regions[_editRegionIdx].bgImage = '';
      await _saveEditingWV(w);
    }
  }
...
${settingText ? settingText.slice(0, 1500) : '（未提供）'}`;
      ...
      // 去 ``` 代码块包裹
      if (cleaned.startsWith('```')) cleaned = cleaned.replace(/^```\w*\n?/, '').replace(/\n?```$/, '').trim();
      // 抓第一个 { 到最后一个 }，剥离前后混入的叙述文字
      const fb = cleaned.indexOf('{');
      const lb = cleaned.lastIndexOf('}');
      if (fb !== -1 && lb > fb) cleaned = cleaned.substring(fb, lb + 1);
      // 修复常见 JSON 瑕疵：对象/数组末尾的多余逗号
      cleaned = cleaned.replace(/,\s*([}\]])/g, '$1');
      let c;
      try { c = JSON.parse(cleaned); }
  ...
  async function _syncRuntime(w) {
    ...
    try {
      ...
      (w.regions || []).forEach(reg => {
          flatRegions.push({ id: reg.id, name: reg.name, summary: reg.summary, detail: reg.detail, bgImage: reg.bgImage });
...
function _buildPhoneAppsEditorHTML(w) {
  // 限定货币多选：从世界观已定义的自定义属性里选，留空=不限制
  const _gAttrs = (w && w.gameplay && Array.isArray(w.gameplay.globalAttrs)) ? w.gameplay.globalAttrs.filter(a => a && a.id) : [];
  const _pa0 = (w && w.phoneApps) || {};
  const _curBlock = (mallKey, boundIds) => {
    const _ids = Array.isArray(boundIds) ? boundIds : [];
    if (!_gAttrs.length) {
      return `<div style="margin-top:10px;font-size:11px;color:var(--text-secondary);line-height:1.5">限定货币：世界观还没定义自定义属性，无法限定。<br>可先去玩法配置里添加属性。</div>`;
    }
    const chips = _gAttrs.map(a => {
      const checked = _ids.includes(a.id) ? 'checked' : '';
      return `<label style="display:inline-flex;align-items:center;gap:5px;padding:5px 10px;border:1px solid var(--border);border-radius:999px;cursor:pointer;font-size:12px;color:var(--text);background:var(--bg-secondary)">
        <input type="checkbox" class="pa-cur-chk" data-mall="${Utils.escapeHtml(mallKey)}" value="${Utils.escapeHtml(a.id)}" ${checked} style="width:14px;height:14px;accent-color:var(--accent);flex-shrink:0">
        <span>${Utils.escapeHtml(a.name || a.id)}</span>
      </label>`;
    }).join('');
    return `<div style="margin-top:10px;padding-top:10px;border-top:1px dashed var(--border)">
      <span style="display:block;font-size:12px;color:var(--text);margin-bottom:6px">限定货币 <span style="font-size:11px;color:var(--text-secondary)">（勾选后本商城只收这几种货币，留空=不限制）</span></span>
...
<div style="flex:1;overflow-y:auto;padding:16px">
  ...
  <div style="background:var(--bg-tertiary);padding:12px;border-radius:8px;margin-bottom:16px">
    ...
    ${_curBlock('takeout', (_pa0.takeout || {}).currencyIds)}
  </div>
  ...
  <div style="background:var(--bg-tertiary);padding:12px;border-radius:8px;margin-bottom:16px">
    ...
    ${_curBlock('shop', (_pa0.shop || {}).currencyIds)}
  </div>
  ...
  <!-- 地图 -->
  <div style="font-size:14px;font-weight:600;color:var(--text);margin-bottom:4px;display:flex;align-items:center;gap:6px">
    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="1 6 1 22 8 18 16 22 23 18 23 2 16 6 8 2 1 6"/><line x1="8" y1="2" x2="8" y2="18"/><line x1="16" y1="6" x2="16" y2="22"/></svg>
    地图
    <span style="font-size:11px;font-weight:normal;color:var(--text-secondary)">（默认：高德）</span>
  </div>
  <div style="background:var(--bg-tertiary);padding:12px;border-radius:8px;margin-bottom:16px">
    <label style="display:block;margin-bottom:10px">
      <span style="display:block;font-size:12px;color:var(--text);margin-bottom:4px">APP 名称</span>
      <input type="text" id="pa-map-name" placeholder="例如：山河舆图 / 星轨导航" style="width:100%;padding:6px 10px;background:var(--bg-secondary);color:var(--text);border:1px solid var(--border);border-radius:6px;font-size:14px">
    </label>
    <label style="display:block">
      <span style="display:block;font-size:12px;color:var(--text);margin-bottom:4px">交通工具描述 <span style="font-size:11px;color:var(--text-secondary)">（告诉地图搜索AI这个世界有哪些交通方式，买票会进物品栏）</span></span>
      <textarea id="pa-map-desc" class="auto-resize-textarea" rows="3" placeholder="例如：修真界出行靠御剑飞行、传送阵、灵兽坐骑、宗门穿梭符，长途可乘飞舟" style="width:100%;padding:8px 10px;background:var(--bg-secondary);color:var(--text);border:1px solid var(--border);border-radius:6px;font-size:14px;line-height:1.5;resize:vertical;min-height:60px"></textarea>
    </label>
    ${_curBlock('map', (_pa0.map || {}).currencyIds)}
  </div>
...
<div>
  ...
  <div style="background:var(--bg-tertiary);padding:12px;border-radius:8px;margin-bottom:16px">
    ...
    <div style="font-size:12px;font-weight:600;color:var(--text-secondary);margin-top:8px;padding-top:8px;border-top:1px solid var(--border)">直播间商城</div>
    ${_curBlock('live', (_pa0.video || {}).liveCurrencyIds)}
  </div>
  ...
  <div style="background:var(--bg-tertiary);padding:12px;border-radius:8px;margin-bottom:16px">
    ...
    ${_curBlock('wardrobe', (_pa0.wardrobe || {}).currencyIds)}
  </div>
...
async function closePhoneAppsEditor() {
  ...
  if (w) {
    ...
    w.phoneApps.map.desc = getVal('pa-map-desc');
    ...
    // 限定货币：收集各商城勾选的自定义属性 id（空数组=不限制）
    {
      const collectCur = (mallKey) => {
        const arr = [];
        document.querySelectorAll(`.pa-cur-chk[data-mall="${mallKey}"]`).forEach(el => {
          if (el.checked && el.value) arr.push(el.value);
        });
        return arr;
      };
      w.phoneApps.takeout.currencyIds = collectCur('takeout');
      w.phoneApps.shop.currencyIds = collectCur('shop');
      w.phoneApps.cottage.currencyIds = collectCur('cottage');
      w.phoneApps.wardrobe.currencyIds = collectCur('wardrobe');
      w.phoneApps.video.liveCurrencyIds = collectCur('live');
      w.phoneApps.map.currencyIds = collectCur('map');
    }
